...
-- Generated: 2026-03-23T01:40:50.650619
-- Database: postgres
...
CREATE TABLE IF NOT EXISTS public.photos (
    ...
    upload_status upload_status NOT NULL DEFAULT 'PENDING_UPLOAD'::upload_status,
    created_at timestamp with time zone NOT NULL DEFAULT now(),
    updated_at timestamp with time zone NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.user_projects (
    ...
    CONSTRAINT user_projects_pkey PRIMARY KEY (user_id),
...
CREATE UNIQUE INDEX uq_contractors_name_ci ON public.contractors USING btree (lower(name));
...
CREATE UNIQUE INDEX uq_own_forces_name_ci ON public.own_forces USING btree (lower(name));
...
CREATE UNIQUE INDEX uq_work_types_name_ci ON public.work_types USING btree (lower(name));

...
Перенести существующие объекты можно двумя способами.
...
UI ходит через Edge Function `sign` и presigned URL'ы — это удобно для
небольших объёмов (десятки–сотни объектов).

#### Вариант B: CLI-миграция (рекомендуется для больших объёмов)

Утилита `scripts/migrate-storage/cli.mjs` ходит в R2 / Cloud.ru
**напрямую** (с service_role и S3-ключами), параллелит работу и устойчива
к перезапускам. Состоит из трёх фаз:

```bash
# 1. Проверить окружение, доступ к Supabase / R2 / Cloud.ru, посчитать объём
npm run migrate:storage:check

# 2. Перенос (идемпотентно — можно перезапускать)
npm run migrate:storage:run
# Опции: --concurrency=8 --limit=100 --dry-run --only=photos|plans

# 3. Проверить, что всё легло на Cloud.ru
npm run migrate:storage:verify
# Опции: --sample=200 --compare-r2
```

Перед первым запуском скопируйте `scripts/migrate-storage/.env.migrate.example`
в корневой `.env.migrate` и заполните `SUPABASE_SERVICE_ROLE_KEY`,
`R2_*` и `CLOUDRU_*`. Полная документация — в
[scripts/migrate-storage/README.md](scripts/migrate-storage/README.md).

Обе миграции идемпотентны: повторный запуск перенесёт только то, что не
...
- **Перенос данных**: после деплоя либо зайти администратором в `/admin/storage-migration` (UI), либо запустить CLI `npm run migrate:storage:check && npm run migrate:storage:run && npm run migrate:storage:verify` (для больших объёмов). См. секцию «Перенос с Cloudflare R2 на Cloud.ru» и [scripts/migrate-storage/README.md](scripts/migrate-storage/README.md).

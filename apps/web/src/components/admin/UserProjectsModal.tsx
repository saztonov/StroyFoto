      ...
      <div className="max-h-[80vh] w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-xl">
        ...
        <div className="max-h-[50vh] overflow-y-auto px-6 py-4">
          {loading ? (
            ...
            <p className="py-4 text-center text-sm text-gray-400">Нет активных проектов</p>
          ) : (
            <div className="space-y-2">
              {projects.map((project) => (
                <label
                  key={project.id}
                  className="flex cursor-pointer items-center gap-3 rounded-lg border border-gray-200 px-4 py-3 hover:bg-gray-50 transition"
                >
                  <input
                    type="checkbox"
                  ...
                  <div className="flex-1">
                    ...
                    <span className="ml-2 text-xs text-gray-400">({project.code})</span>
                  </div>
                </label>
              ))}
            </div>

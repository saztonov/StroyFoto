...
- Refresh-токен сохраняется в одном из двух мест в зависимости от чекбокса
  «Запомнить меня» на форме логина ([src/pages/auth/LoginPage.tsx](src/pages/auth/LoginPage.tsx)):
  - **снят** (default): `sessionStorage` ключ `stroyfoto:auth_session` —
    сессия гасится при закрытии вкладки/окна, требуется повторный ввод email/пароля.
  - **установлен**: IndexedDB store `auth_session` — автологин до
    `refreshExpiresAt` (30 дней по умолчанию).

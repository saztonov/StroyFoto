...
| Presign | Supabase Edge Function `sign` (aws4fetch SigV4, провайдер `cloudru`/`r2`) |
...
Backend-API нет. Вся логика во фронтенде; секреты Cloud.ru S3 (и при необходимости R2 для миграции) хранятся только в Edge Function.
...
Browser/PWA
 ...
 ├─► Edge Function sign → presigned URL (Cloud.ru S3 / R2 fallback для миграции)
 └─► Cloud.ru Object Storage (PUT/GET фото и PDF)
```
...
│   ├── 20260427_work_assignments.sql        # Назначения работ
│   └── 20260501_cloudru_storage.sql         # Колонка `storage` (cloudru | r2)
├── config.toml               # Supabase CLI config
└── functions/sign/index.ts   # Edge Function: presigned URLs (Cloud.ru S3 + R2 fallback)
```
...
- **Активный провайдер:** Cloud.ru Object Storage (`https://s3.cloud.ru`,
  ru-central-1). Подпись presigned URL делает Edge Function `sign` через
  aws4fetch SigV4. accessKeyId формируется как `${tenant_id}:${key_id}`.
- **Колонка `storage`** (`cloudru` \| `r2`) в `report_photos` и `plans`
  определяет, к какому endpoint'у обращаться. Default — `'cloudru'`.
- **Параметр `provider`** в теле запроса к Edge Function `sign` — `cloudru`
  по умолчанию, `r2` разрешён только админу и только для GET (для миграции).
- **Миграция** (`/admin/storage-migration`, `src/services/storageMigration.ts`):
  построчный copy R2 → Cloud.ru с обновлением `storage='cloudru'`.
  Идемпотентна, можно перезапускать; object key не меняется.
...
1. **Секреты:** никогда не класть service_role key или ключи объектного хранилища (Cloud.ru / R2) в клиент. Только presigned URL через Edge Function.
